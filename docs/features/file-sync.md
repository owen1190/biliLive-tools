...
使用 biliLive-tools 自带的录播器时，还需要在录播任务中打开“发送至 webhook”。录制文件完成后，程序会自动发出 `FileClosed` 事件，随后按房间配置的同步器上传。

配置顺序：

1. 在“设置 -> 文件同步”中配置网盘客户端并完成“登录检查”和“上传测试”。
2. 在“设置 -> Webhook”中打开全局 webhook，并创建或选择一个同步器；同步文件类型至少勾选“源文件”。
3. 在对应直播间的录播设置中打开“发送至 webhook”。

如果只希望上传弹幕压制后的版本，在同步器中勾选“弹幕压制后的文件”；如果同时勾选“源文件”和“弹幕压制后的文件”，两种版本都会上传。
